...
(function () {
  ...
  <div class="relative mx-auto my-6 md:my-10 w-[min(95vw,1200px)] h-[min(90vh,900px)]">
    <div class="bg-white rounded-2xl shadow-xl w-full h-full overflow-hidden flex flex-col md:flex-row">
      <button type="button"
              class="absolute top-3 right-3 bg-black/80 text-white rounded-full w-9 h-9 grid place-items-center hover:bg-black focus:outline-none z-10"
              aria-label="Close"
              onclick="closeProductModal()">&#10005;</button>

      <!-- Image takes more space for "full glory" display -->
      <div class="h-[60vh] md:w-[65%] md:h-full bg-gray-50 flex items-center justify-center p-3 md:p-6">
        <img id="pmImg" class="w-full h-full object-cover rounded-lg" src="" alt="">
      </div>

      <!-- Text container - visible but compact -->
      <div class="flex-1 md:w-[35%] p-4 md:p-6 h-[40vh] md:h-full overflow-y-auto border-t md:border-t-0 md:border-l border-gray-200">
        <h3 id="pmTitle" class="text-xl md:text-2xl font-bold mb-2"></h3>
        <div id="pmCat" class="text-sm text-gray-600 mb-4"></div>
        <p id="pmDesc" class="text-gray-700 text-sm md:text-base"></p>
      </div>
    </div>
  </div>
  `;
